...
/**
 * Configuration class to conditionally provide the AdminService implementation
...
public class AuthServiceConfig {
     ...
     * @return The AdminService implementation for Keycloak.
     */
    @Bean
    @ConditionalOnProperty(name = "auth.server.type", havingValue = "keycloak")
    public AdminService keycloakAdminService(KeycloakAdminServiceImpl keycloakAdminServiceImpl) {
        return keycloakAdminServiceImpl;
    }

    // You could add other @Bean methods for other AdminService implementations here, e.g.:
    // @Bean
    // @ConditionalOnProperty(name = "auth.server.type", havingValue = "auth0")
    // public AdminService auth0AdminService(Auth0AdminServiceImpl auth0AdminServiceImpl) {
